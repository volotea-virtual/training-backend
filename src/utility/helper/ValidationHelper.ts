            ...
            switch (val.val) {
                ...
                case ValidationOptions.NON_NULL:
                    if (toCheck == null || toCheck.length == 0) {
                        invalid = true;
                        message.push({key: opt.name, code: "VAL_NULL", message: "The paramenter is NULL, although is not allowed to be NULL"});
                    }
                    break;
                    ...
                    case ValidationOptions.MIN_LENGHT:
                    if(toCheck.length <= val.value) {
                        invalid = true;
                    ...
                    if ( Number.isNaN(n)) {
                        invalid = true;
                        message.push({key: opt.name, code: "VAL_NAN", message: "Paramter is not a number."});
                    }
                    break;
